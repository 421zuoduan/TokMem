...
run_name=tapmem_llama8b_residualroute_w005_additive_e100_seed42_v1
...
cd "$repo_dir"
echo "[$(date --iso-8601=seconds)] starting route-weight-0.05 TapMem training on GPU $train_gpu"
CUDA_VISIBLE_DEVICES="$train_gpu" "$python_bin" -u \
  ...
  --route-loss-weight 0.05 \
  --routing-target-mode residual_set_margin \
  --routing-candidate-mass 0.9 \
  --routing-margin 0.5 \
...
for attempt in 1 2 3; do
    ...
    if CUDA_VISIBLE_DEVICES=1 "$python_bin" \
        -m compositional_intercode_bash.evaluate \
        --checkpoint "$checkpoint_dir" \
        --output-dir "$evaluation_dir" \
        --max-turns 10 \
