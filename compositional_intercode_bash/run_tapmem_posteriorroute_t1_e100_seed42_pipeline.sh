...
run_name=tapmem_llama8b_posteriorroute_t1_e100_seed42_v1
...
cd "$repo_dir"
echo "[$(date --iso-8601=seconds)] starting 100-epoch TapMem training on GPU $train_gpu"
CUDA_VISIBLE_DEVICES="$train_gpu" "$python_bin" -u \
  ...
  --routing-target-mode fixed_count_posterior \
