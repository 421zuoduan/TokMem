...
SUITE_NAME="paper_compositional_logit_bias_scale_ablation_llama1b_4calls_${SUITE_TIMESTAMP}"
GPU_IDS_CSV="0,1,2,3,4,5,6,7"
POLL_SECONDS=5
GPU_MEMORY_LIMIT_MIB=2048
GPU_IDLE_REQUIRED_SECONDS=180
GPU_LOCK_DIR="/tmp/tokmem_gpu_locks"

usage() {
    cat <<EOF
Usage: bash scripts/compositional/llama_1b/rerun_paper_compositional_logit_bias_scale_ablation.sh [--gpus 0,1,2,3,4,5,6,7] [--suite-name NAME] [--poll-seconds N] [--gpu-memory-limit-mib N] [--gpu-idle-required-seconds N]

Runs the compositional logit-bias scale ablation for:
- model: llama1b
- method: tokmem_eoc_logit_bias
- tools: 51-100
- call scope: 4 calls
- scales: 0.1, 0.5, 0.8, 1, 1.1, 1.2, 1.3, 1.4, 1.5, 2, 3, 5
- 3 trials per scale, seed fixed to 42 for every trial
- GPU list is used as worker slots; extra scales are queued round-robin.
- each GPU worker takes a per-GPU flock lock, then starts after memory.used stays <= 2048 MiB for 180 consecutive seconds.
...
printf "scale\tgpu\ttrial\tseed\tstatus\texit_code\trun_name\trun_dir\tevaluation_results\ttraining_summary\tstdout_log\n" > "$MANIFEST_FILE"
...
run_scale() {
    ...
    local scale_status=0
    mkdir -p "$scale_dir"

    export CUDA_VISIBLE_DEVICES="$gpu"

    for trial in "${TRIALS[@]}"; do
        local run_name="tokmem_eoc_logit_bias_scale${slug}_llama_1b_4calls_seed42_3x_${SUITE_TIMESTAMP}_trial${trial}"
        ...
        (
            ...
            python -u main_sequential.py \
                ...
                --run_name "$run_name" \
                --run_tag "llama1b_tokmem_eoc_logit_bias_scale${slug}_4calls"
        ) 2>&1 | tee "$stdout_log"
...
scale_labels = ", ".join(f"`{scale}`" for scale in sorted(scale_summaries, key=float))
gpu_assignments = ", ".join(
    f"`{scale} -> {scale_summaries[scale]['trials'][0]['gpu']}`"
    for scale in sorted(scale_summaries, key=float)
    if scale_summaries[scale]["trials"]
)
...
lines = [
    ...
    "- logit-bias settings: `detach=true`, `use_logit_train_add=true`, `logit_bias_loss_weight=0.1`, `logit_bias_network=linear`",
    ...
    "## Mean Metrics",
    "",
        "| Scale | Routing Acc | Routing Std | Arguments F1 | Arguments F1 Std | Tool F1 | Exact Match | Count |",
    "| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
...
lines.extend(
    [
        ...
        "## Trial Details",
        "",
        "| Scale | GPU | Trial | Status | Exit Code | Run | Routing Acc | Arguments F1 | Tool F1 | Exact Match |",
        "| ---: | ---: | ---: | --- | ---: | --- | ---: | ---: | ---: | ---: |",
    ]
)
for scale, summary in sorted(scale_summaries.items(), key=lambda item: float(item[0])):
...
run_gpu_queue() {
    ...
    (
        ...
        for scale_index in "${!SCALES[@]}"; do
            if (( scale_index % gpu_count == gpu_index )); then
                if ! run_scale "${SCALES[$scale_index]}" "$gpu"; then
                    worker_status=1
                fi
            fi
        done
