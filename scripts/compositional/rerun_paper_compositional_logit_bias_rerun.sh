...
SUITE_NAME="paper_compositional_head_51_100_4calls_10calls_${SUITE_TIMESTAMP}"
GPU_IDS_CSV="4,5,6,7"
POLL_SECONDS=5
...
usage() {
    cat <<EOF
Usage: bash scripts/compositional/rerun_paper_compositional_head.sh [--gpus 0,1,2,3] [--suite-name NAME] [--poll-seconds N] [--rerun-failed]

Runs the logit-bias subset of the compositional paper suite for:
- tools 51-100
- 4 calls logit-bias comparison
- 10 calls logit-bias stress test with separate result tables
- models: llama1b, llama3b, llama8b
- methods: tokmem_eoc_logit_bias, adap_tokmem_eoc_logit_bias
- 3 trials per model/method, seed fixed to 42 for every trial
...
- rerunning an existing --suite-name reconciles the manifest with the logit-bias method set
- --rerun-failed reuses the existing manifest and retries unfinished logit-bias tasks already recorded there
EOF
}
    ...
    case "$method" in
        ...
        adap_tokmem|adap_tokmem_eoc|adap_tokmem_eoc_logit_bias|adap_tokmem_eoc_replace_head)
            ...
            if [[ "$method" == "adap_tokmem_eoc_logit_bias" ]]; then
                TASK_CMD+=(--use_logit_bias --detach --use_logit_train_add)
            fi
...
is_training_method() {
    local method="$1"
    case "$method" in
        tokmem_eoc_logit_bias|adap_tokmem_eoc_logit_bias)
            return 0
            ;;
        *)
            return 1
...
is_method_enabled_for_scope() {
    local call_scope="$1"
    local method="$2"
    local enabled_method

    for enabled_method in ${METHODS_BY_SCOPE[$call_scope]-}; do
        if [[ "$method" == "$enabled_method" ]]; then
            return 0
        fi
    done
...
load_task_manifest() {
    ...
    while IFS=$'\t' read -r task_id call_scope model_key method trial seed training_flag task_name task_dir; do
        ...
        if ! is_method_enabled_for_scope "$call_scope" "$method"; then
            continue
        fi
        append_task_record \
            "$task_id" \
...
enabled_methods_by_scope = {
    "4calls": {"tokmem_eoc_logit_bias", "adap_tokmem_eoc_logit_bias"},
    "10calls": {"tokmem_eoc_logit_bias", "adap_tokmem_eoc_logit_bias"},
}
tasks = []
for row in rows:
    call_scope = row.get("call_scope") or "4calls"
    if row["method"] not in enabled_methods_by_scope.get(call_scope, set()):
        continue
...
rows = [
    row
    for row in csv.DictReader(manifest_path.open("r", encoding="utf-8"), delimiter="\t")
    if row["method"] in enabled_methods_by_scope.get(row.get("call_scope") or "4calls", set())
]
...
summary_lines = [
    f"# Compositional Logit-Bias Head Suite Summary",
    "",
    f"- suite: `{suite_name}`",
    ...
    "- models: `llama1b`, `llama3b`, `llama8b`",
    "- methods: `tokmem_eoc_logit_bias`, `adap_tokmem_eoc_logit_bias`",
    "- learning rates: `tokmem_lr=5e-3`, `adaptation_lora_lr=8e-5`",
    ...
    "- adaptation 4calls scope: `adap_tokmem_eoc_logit_bias` uses `1-50:1,51-100:3` with 4 calls in both rounds",
    "- adaptation 10calls scope: `adap_tokmem_eoc_logit_bias` uses `1-50:1,51-100:3` with `4,10` calls by round",
    "",
    "## Batch Settings",
    "",
    "- `tokmem_eoc_logit_bias`: `llama1b train/eval=24/256`, `llama3b train/eval=16/192`, `llama8b train/eval=8/64`",
    "- `tokmem_eoc_logit_bias` 10calls: `llama1b train/eval=4/16`, `llama3b train/eval=2/8`, `llama8b train/eval=1/4`",
    "- `adap_tokmem_eoc_logit_bias`: `llama1b train=16,24 eval=256`, `llama3b train=8,16 eval=192`, `llama8b train=4,8 eval=64`, rounds=`1-50:1,51-100:3`",
    "- `adap_tokmem_eoc_logit_bias` 10calls: `llama1b train=16,4 eval=16`, `llama3b train=8,2 eval=8`, `llama8b train=4,1 eval=4`, rounds=`1-50:1,51-100:3`",
]
...
summary_lines.extend(
    [
        ...
        "## Notes",
        "",
        f"- Mean metrics are reported for groups with `{trial_count}/{trial_count}` successful trials.",
