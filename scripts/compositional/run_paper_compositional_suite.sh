...
SUITE_NAME="paper_compositional_51_100_4calls_${SUITE_TIMESTAMP}"
GPU_IDS_CSV="4,5,6,7"
POLL_SECONDS=5
RERUN_FAILED=0
...
- methods: icl, rag, lora, tokmem, tokmem_eoc, tokmem_eoc_logit_bias
...
METHODS=(icl rag lora tokmem tokmem_eoc tokmem_eoc_logit_bias)
TRAINING_METHODS=(lora tokmem tokmem_eoc tokmem_eoc_logit_bias)
...
write_suite_config() {
    ...
    local rerun_id="$3"
    python - "$output_path" "$SUITE_NAME" "$GPU_IDS_CSV" "$POLL_SECONDS" "$RERUN_FAILED" "$metadata_scope" "$rerun_id" <<'PY'
import json
import sys
...
payload = {
    ...
    "metadata_scope": sys.argv[6],
    "rerun_id": sys.argv[7] or None,
    "dataset": {
        "top_k": "51-100",
        "train_size": 5000,
        "test_size": 500,
    ...
    "batch_settings": {
        "tokmem_family": {
            "llama1b": {"train_batch_size": 32, "eval_batch_size": 256},
...
is_training_method() {
    local method="$1"
    case "$method" in
        lora|tokmem|tokmem_eoc|tokmem_eoc_logit_bias)
            return 0
            ;;
        *)
            return 1
...
load_task_manifest() {
    local manifest_path="$1"
    local task_id model_key method trial seed training_flag task_name task_dir

    while IFS=$'\t' read -r task_id model_key method trial seed training_flag task_name task_dir; do
...
prepare_task_manifest() {
    ...
    if [[ "$RERUN_FAILED" -eq 1 && -f "$MANIFEST_FILE" ]]; then
        log "Reusing existing task manifest: $MANIFEST_FILE"
...
launch_task() {
    ...
    mkdir -p "$task_dir"
    build_task_command "$model_key" "$method" "$task_name" "$seed"
...
summary_lines = [
    ...
    "## Batch Settings",
    "",
    "- `tokmem*`: `llama1b train/eval=32/256`, `llama3b train/eval=16/192`, `llama8b train/eval=8/64`",
...
summary_lines.extend(
    [
        ...
        "- Training runtime ranking is sorted by average successful-trial runtime among training methods only.",
