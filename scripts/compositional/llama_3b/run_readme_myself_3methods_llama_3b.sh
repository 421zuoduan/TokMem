...
RUN_NAME="readme_myself_3methods_llama_3b_${RUN_ID}"
...
MODEL_DIR="$ROOT_DIR/models/Llama-3.2-3B-Instruct"
...
for setting_entry in "${SETTINGS[@]}"; do
    ...
    for seed in "${TRIAL_SEEDS[@]}"; do
        ...
        trial_name="readme_myself_3methods_llama_3b_setting${setting_id}_trial${trial_index}_${RUN_ID}"
        ...
        cmd=(
            ...
            --run_name "$trial_name"
            --run_tag "readme_myself_3methods_llama_3b_setting${setting_id}"
        )
...
summary_lines = [
    f"# README_MYSELF 三种维护方法对比（llama_3b, {trial_count} 次重复均值）",
    "",
    f"- run: `{run_name}`",
    f"- trials per setting: `{trial_count}`",
    "- model: `Llama-3.2-3B-Instruct`",
    "- methods: `baseline`, `eoc-only`, `eoc+logit_bias`",
    "- defaults: `batch_size=2`, `logit_bias_network=linear`, `logit_bias_loss_weight=0.1`, `logit_bias_scale=1.0`",
    "",
    header,
    separator,
]
...
begin_marker = "<!-- README_MYSELF_3METHODS_LLAMA_3B_TABLE:BEGIN -->"
end_marker = "<!-- README_MYSELF_3METHODS_LLAMA_3B_TABLE:END -->"
replacement_lines = [
    begin_marker,
