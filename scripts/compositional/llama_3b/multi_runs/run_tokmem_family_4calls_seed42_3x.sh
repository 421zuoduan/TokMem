...
SUITE_NAME="llama3b_tokmem_family_4calls_seed42_3x_${SUITE_ID}"
SUITE_DIR="$RESULTS_ROOT/$SUITE_NAME"
DATA_DIR="$SUITE_DIR/data"
HF_CACHE_DIR="$SUITE_DIR/hf-cache"
METHOD_RUNS_ROOT="$SUITE_DIR/method_runs"
...
METHOD_NAMES=(
    tokmem_llama_3b
    tokmem_eoc_llama_3b
    tokmem_eoc_logit_bias_llama_3b
)

METHOD_SCRIPTS=(
    scripts/compositional/llama_3b/tokmem_llama_3b_4calls_seed42_3x.sh
    scripts/compositional/llama_3b/tokmem_eoc_llama_3b_4calls_seed42_3x.sh
    scripts/compositional/llama_3b/tokmem_eoc_logit_bias_llama_3b_4calls_seed42_3x.sh
)
...
mkdir -p "$SUITE_DIR" "$METHOD_RUNS_ROOT" "$HF_CACHE_DIR"
cp "$SCRIPT_PATH" "$SUITE_DIR/$(basename "$SCRIPT_PATH")"

printf "method_key\tmethod_name\tscript\tgpu\tsuite_dir\tresults_json\tsummary_md\tstdout_log\n" > "$MANIFEST_FILE"
...
for idx in "${!METHOD_KEYS[@]}"; do
    ...
    echo "[method $method_key] launching $method_script on GPU $method_gpu"
    (
        CUDA_VISIBLE_DEVICES="$method_gpu" \
        TOKMEM_DATA_DIR="$DATA_DIR" \
        TOKMEM_HF_CACHE_DIR="$HF_CACHE_DIR" \
        TOKMEM_RESULTS_ROOT="$METHOD_RUNS_ROOT" \
        TOKMEM_SUITE_ID="$SUITE_ID" \
...
for idx in "${!METHOD_KEYS[@]}"; do
    ...
    if wait "${METHOD_PIDS[$idx]}"; then
        echo "[method $method_key] completed"
    else
        echo "[method $method_key] failed; see $stdout_log" >&2
        failed=1
        continue
    fi
...
python - "$MANIFEST_FILE" "$SUMMARY_FILE" "$ARTIFACTS_FILE" "$SUITE_NAME" "$DATA_DIR" <<'PY'
import csv
import json
import sys
...
summary_lines = [
    ...
    "- GPUs: `tokmem=4`, `tokmem_eoc=5`, `tokmem_eoc_logit_bias=6`",
    "- paper-suite aligned settings: `training_rounds=51-100:1`, `epochs=3`, `batch_size=16`, `eval_batch_size=192`, `max_length=1024`, `lr=5e-3`",
    f"- data dir: `{data_dir}`",
    "",
    "## Methods",
    "",
    "| Method | GPU | Flags | Child Suite |",
    "| --- | ---: | --- | --- |",
]
for method in methods:
    summary_lines.append(
        f"| {method['method_key']} | {method['gpu']} | `{method['method_flags']}` | `{Path(method['suite_dir']).name}` |"
    )
