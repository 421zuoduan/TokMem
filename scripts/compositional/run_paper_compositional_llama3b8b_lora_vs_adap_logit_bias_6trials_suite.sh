...
SUITE_NAME="paper_compositional_llama3b8b_lora_vs_adap_logit_bias_4calls_6trials_${SUITE_TIMESTAMP}"
GPU_IDS_CSV="4,5,6,7"
POLL_SECONDS=5
...
usage() {
    cat <<EOF
Usage: bash scripts/compositional/run_paper_compositional_llama3b8b_lora_vs_adap_logit_bias_6trials_suite.sh [--gpus 0,1,2,3] [--suite-name NAME] [--poll-seconds N] [--rerun-failed]

Runs the compositional 4-call rerun suite for:
- tools 51-100
- 4 calls comparison
- models: llama3b, llama8b
- methods: lora, adap_tokmem_eoc_logit_bias
- 6 trials per model/method, seed fixed to 42 for every trial
...
- rerunning an existing --suite-name reconciles the manifest with the lora/logit-bias method set
...
generate_dataset() {
    ...
    if [[ -f "$pretrain_train_file" && -f "$pretrain_test_file" && -f "$pretrain_tool_file" \
        && -f "$train_file_4calls" && -f "$test_file_4calls" && -f "$tool_file" ]]; then
...
is_training_method() {
    local method="$1"
    case "$method" in
        lora|adap_tokmem_eoc_logit_bias)
            return 0
            ;;
        *)
            return 1
...
harvest_finished_tasks() {
    ...
    for gpu_id in "${GPU_IDS[@]}"; do
        ...
        unset GPU_EXTERNAL_COOLDOWN_REQUIRED["$gpu_id"]
        GPU_IDLE_SINCE["$gpu_id"]=$(($(date +%s) - GPU_IDLE_REQUIRED_SECONDS))
        harvested_any=1
    done
...
summary_lines = [
    f"# Llama3B/8B LoRA And Adaptation Logit-Bias Rerun Summary",
    "",
    f"- suite: `{suite_name}`",
    f"- trials per model/method: `{trial_count}`",
    "- evaluation scope: `tools 51-100 / 4 calls`",
    "- max length: `512`",
    "- models: `llama3b`, `llama8b`",
    "- methods: `lora`, `adap_tokmem_eoc_logit_bias`",
    ...
    "- adaptation scope: `adap_tokmem_eoc_logit_bias` uses `1-50:1,51-100:3` with 4 calls in both rounds",
    "",
    "## Batch Settings",
    "",
    "- `lora`: `llama3b train/eval=8/96`, `llama8b train/eval=4/32`, rounds=`51-100:3`, lr=`5e-5`",
    "- `adap_tokmem_eoc_logit_bias`: `llama3b train=8,16 eval=192`, `llama8b train=4,8 eval=64`, rounds=`1-50:1,51-100:3`, tokmem lr=`5e-3`, adaptation lora lr=`8e-5`",
]
...
summary_lines.extend(
    [
        ...
        "- Incomplete groups are listed separately.",
        "- Training runtime ranking is sorted by average successful-trial runtime.",
