...
SUITE_NAME="atomic_llama8b_tokmem_train_add_detach_3trials_${SUITE_TIMESTAMP}"
GPU_IDS_CSV="0,1,2,3"
POLL_SECONDS=5
...
usage() {
    cat <<EOF
Usage: bash scripts/atomic/run_atomic_llama8b_tokmem_train_add_detach_3trials.sh [--gpus 0,1,2,3] [--suite-name NAME] [--poll-seconds N] [--rerun-failed] [--num-tasks 700] [--split-cache PATH]

Runs the maintained fewer-time atomic fixed-split llama8b TokMem-family suite for:
...
- Each launched task holds a per-GPU flock lock under /tmp/tokmem_gpu_locks for the full process lifetime.
...
SPLIT_CACHE="$ROOT_DIR/atomic/cached_splits/task700-80-10-20-seed42/tokmem_atomic_fixed_split_maxlen1024.pt"
...
gpu_lock_file() {
    local gpu_id="$1"
    local lock_name="${gpu_id//[^A-Za-z0-9_.-]/_}"
    echo "$GPU_LOCK_DIR/gpu_${lock_name}.lock"
}

write_suite_config() {
    local trial_seeds_csv
    trial_seeds_csv="$(IFS=,; echo "${TRIAL_SEEDS[*]}")"
    ...
    python - "$SUITE_CONFIG_FILE" "$SUITE_NAME" "$GPU_IDS_CSV" "$POLL_SECONDS" "$GPU_MEMORY_LIMIT_MIB" \
        ...
        "$MAX_LENGTH" "$MAX_INSTRUCTION_TOKENS" "$MAX_NEW_TOKENS" \
        "$TRIAL_COUNT" "$trial_seeds_csv" "$GPU_LOCK_DIR" <<'PY'
import json
import sys
...
payload = {
    ...
    "trial_count": int(sys.argv[21]),
    "trial_seeds": [int(item) for item in sys.argv[22].split(",") if item],
    "logit_bias_settings": {
        "detach": True,
...
launch_task() {
    ...
    local lock_file
    lock_file="$(gpu_lock_file "$gpu_id")"
...
EOF
    ...
    (
        set +e
        log "Waiting for GPU lock gpu=$gpu_id lock_file=$lock_file task=$task_name"
        flock -x 200
        local_lock_exit_code=$?
        if [[ "$local_lock_exit_code" -ne 0 ]]; then
            log "GPU lock failed gpu=$gpu_id lock_file=$lock_file task=$task_name exit_code=$local_lock_exit_code"
            exit "$local_lock_exit_code"
        fi
        log "GPU lock acquired gpu=$gpu_id lock_file=$lock_file task=$task_name"
        wait_for_locked_gpu_memory "$gpu_id"
...
wait_for_locked_gpu_memory() {
    local gpu_id="$1"
    local memory_used now_iso

    while true; do
        now_iso="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
        if memory_used="$(gpu_memory_used_mib "$gpu_id")" && [[ "$memory_used" =~ ^[0-9]+$ ]]; then
            if (( memory_used <= GPU_MEMORY_LIMIT_MIB )); then
                printf '[%s]\tgpu=%s\tstate=locked_ready\tmemory_mib=%s\tidle_seconds=0\n' \
                    "$now_iso" "$gpu_id" "$memory_used" >> "$GPU_AVAILABILITY_LOG"
                log "GPU ready under lock gpu=$gpu_id memory_mib=$memory_used limit_mib=$GPU_MEMORY_LIMIT_MIB"
                return 0
            fi

            printf '[%s]\tgpu=%s\tstate=locked_busy\tmemory_mib=%s\tidle_seconds=0\n' \
                "$now_iso" "$gpu_id" "$memory_used" >> "$GPU_AVAILABILITY_LOG"
            log "Waiting for GPU memory under lock gpu=$gpu_id memory_mib=$memory_used limit_mib=$GPU_MEMORY_LIMIT_MIB"
        else
            printf '[%s]\tgpu=%s\tstate=locked_query_failed\tmemory_mib=unknown\tidle_seconds=0\n' \
                "$now_iso" "$gpu_id" >> "$GPU_AVAILABILITY_LOG"
            log "Waiting for GPU memory under lock gpu=$gpu_id reason=nvidia-smi-query-failed"
        fi
...
summary_lines = [
    "# Atomic Llama8B TokMem Train-Add Detach 3-Trial Summary",
    "",
    f"- suite: `{suite_name}`",
