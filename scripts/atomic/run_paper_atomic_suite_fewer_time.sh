...
SUITE_NAME="paper_atomic_700task_80_10_20_2trials_${SUITE_TIMESTAMP}"
GPU_IDS_CSV="0,1,2,3"
POLL_SECONDS=5
...
usage() {
    cat <<EOF
Usage: bash scripts/atomic/run_paper_atomic_suite_fewer_time.sh [--gpus 0,1,2,3] [--suite-name NAME] [--poll-seconds N] [--rerun-failed] [--num-tasks 700] [--split-cache PATH]

Runs the maintained fewer-time atomic fixed-split suite for:
...
- 2 trials per model/method, seed fixed to 42 for every trial to match the cached split metadata
...
write_suite_config() {
    ...
    python - "$SUITE_CONFIG_FILE" "$SUITE_NAME" "$GPU_IDS_CSV" "$POLL_SECONDS" "$GPU_MEMORY_LIMIT_MIB" \
        ...
        "$TRIAL_COUNT" "$trial_seeds_csv" <<'PY'
import json
import sys
