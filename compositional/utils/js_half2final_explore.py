...
def setup_logger(output_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("js_half2final_explore")
...
def derive_output_dir(checkpoint_path: Path, requested_output_dir: Optional[str]) -> Path:
    ...
    return REPO_ROOT / "compositional" / "runs" / "js_half2final_explore" / f"{checkpoint_stem}_{timestamp}"
...
def compute_half2final_js_metrics(js_curve: Sequence[float]) -> Dict[str, float]:
    ...
    half_curve = first_half if first_half.size else curve
...
def analyze_hidden_states_with_logit_lens(
    ...
    # and define js_mean/js_std only over the first half of layers.
    ...
    return js_curve, compute_half2final_js_metrics(js_curve)
...
def plot_single_sample(
    ...
    left_ax.set_title("Half-to-final JS Mean +/- Std")
    ...
    # Plot semantics:
    # left: per-token half-to-final JS mean with first-half JS std error bars.
    # right: per-category aggregate half-to-final JS mean comparison.
...
def plot_global_comparison(global_summary: Dict[str, Any], output_path: Path) -> None:
    ...
    ax.set_title("Half-to-final JS Mean")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Re-evaluate a trained compositional checkpoint with online half-to-final JS analysis")
    ...
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory (default: compositional/runs/js_half2final_explore/<timestamp>)")
...
def main() -> None:
    ...
    write_json(
        ...
        {
            "analysis_type": "half_to_final_js_mean",
    ...
    plot_global_comparison(global_summary, output_dir / "plots" / "global_correct_vs_incorrect_half2final_js_mean.png")
    plot_correct_category_comparison(global_summary, output_dir / "plots" / "correct_category_half2final_js_mean.png")
