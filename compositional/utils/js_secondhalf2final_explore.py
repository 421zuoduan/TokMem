...
def setup_logger(output_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("js_secondhalf2final_explore")
...
def derive_output_dir(checkpoint_path: Path, requested_output_dir: Optional[str]) -> Path:
    ...
    return REPO_ROOT / "compositional" / "runs" / "js_secondhalf2final_explore" / f"{checkpoint_stem}_{timestamp}"
...
def build_format_token_id_sets(tokenizer: Any) -> Dict[str, set[int]]:
    def encode_ids(text: str) -> set[int]:
        return set(tokenizer(text, add_special_tokens=False)["input_ids"])

    start_header_ids = encode_ids("<|start_header_id|>")
    end_header_ids = encode_ids("<|end_header_id|>")
    begin_text_ids = encode_ids("<|begin_of_text|>")
    return {
        "start_header_ids": start_header_ids,
        "end_header_ids": end_header_ids,
        "format_token_ids": start_header_ids | end_header_ids | begin_text_ids,
    }
...
def compute_secondhalf2final_js_metrics(js_curve: Sequence[float]) -> Dict[str, float]:
    ...
    second_half = curve[split_index:] if split_index < curve.size else curve
    half_curve = second_half if second_half.size else curve

    return {
        "js_mean": float(half_curve.mean()),
        "js_var": float(half_curve.var()),
        "js_std": float(half_curve.std()),
        "half_layer_count": int(half_curve.size),
    }
...
def summarize_category_metrics(token_metrics: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    metric_names = ["js_mean", "js_var", "js_std"]
    ...
    half_layer_counts = [int(item["half_layer_count"]) for item in token_metrics if "half_layer_count" in item]
    summary["half_layer_count_mean"] = float(np.mean(half_layer_counts)) if half_layer_counts else None
    return summary


def _rank_auc(scores: Sequence[float], labels: Sequence[int]) -> Optional[float]:
    n_pos = sum(labels)
    ...
    while index < len(pairs):
        ...
        rank_sum += avg_rank * sum(label for _, label in pairs[index:next_index])
        rank += next_index - index
        index = next_index
    return (rank_sum - n_pos * (n_pos + 1) / 2.0) / (n_pos * n_neg)


def _cohens_d(focus_values: Sequence[float], other_values: Sequence[float]) -> Optional[float]:
    if len(focus_values) < 2 or len(other_values) < 2:
        return None
    focus_mean = float(np.mean(focus_values))
    other_mean = float(np.mean(other_values))
    focus_var = float(np.var(focus_values, ddof=1))
    other_var = float(np.var(other_values, ddof=1))
    pooled = math.sqrt(
        (((len(focus_values) - 1) * focus_var) + ((len(other_values) - 1) * other_var))
        / (len(focus_values) + len(other_values) - 2)
    )
    if pooled == 0.0:
        return None
    return (focus_mean - other_mean) / pooled


def compare_category_against_others(
    token_records: Sequence[Dict[str, Any]],
    focus_category: str,
    metric_name: str = "js_mean",
) -> Dict[str, Any]:
    focus_values = [float(record[metric_name]) for record in token_records if record["category"] == focus_category]
    other_values = [float(record[metric_name]) for record in token_records if record["category"] != focus_category]
    result: Dict[str, Any] = {
    ...
    if not focus_values or not other_values:
        return result

    focus_mean = float(np.mean(focus_values))
    other_mean = float(np.mean(other_values))
    labels = [1 if record["category"] == focus_category else 0 for record in token_records]
    scores = [float(record[metric_name]) for record in token_records]
    raw_auc = _rank_auc(scores, labels)
    if raw_auc is not None:
        best_auc = max(raw_auc, 1.0 - raw_auc)
        direction = "higher->focus" if raw_auc >= 0.5 else "lower->focus"
    else:
        best_auc = None
        direction = None
    ...
    result.update(
        {
            ...
            "mean_gap": focus_mean - other_mean,
            "best_auc": best_auc,
            "direction": direction,
            "cohens_d": _cohens_d(focus_values, other_values),
        }
    )
    return result
...
def analyze_hidden_states_with_logit_lens(
    ...
    # Second-half-to-final JS rule:
    # project every layer's hidden state with the model's own unembedding / lm_head,
    # compare each layer distribution against the final layer distribution,
    # and define js_mean/js_std only over the second half of layers.
    ...
    return js_curve, compute_secondhalf2final_js_metrics(js_curve)
...
def run_online_generation_capture(
    ...
    format_token_sets = build_format_token_id_sets(tokenizer)
    start_header_ids = format_token_sets["start_header_ids"]
    end_header_ids = format_token_sets["end_header_ids"]
    format_token_ids = format_token_sets["format_token_ids"]
    ...
    with torch.no_grad():
        ...
        current_js_curve = prompt_js_curve
        current_js_metrics = prompt_js_metrics
        ...
        while step < max_new_tokens and not finished:
            ...
            if sampled_token_id in format_token_ids:
                token_is_format = True
            if inside_header_span:
                token_is_format = True
            if sampled_token_id in start_header_ids:
                inside_header_span = True
                token_is_format = True
            ...
            category = classify_generated_token(
                ...
                is_format_token=token_is_format,
            )
            if category is not None:
                token_records.append(
                    build_token_record(
                        ...
                        token_position=step,
                        category=category,
                        js_curve=current_js_curve,
                        js_metrics=current_js_metrics,
                    )
                )
            ...
            next_logits = outputs.logits[:, -1, :].clone()
            past_key_values = outputs.past_key_values
            current_js_curve, current_js_metrics = analyze_hidden_states_with_logit_lens(
                model,
                outputs.hidden_states,
                final_norm_module,
                lm_head_module,
...
def plot_single_sample(
    ...
    left_ax.set_title("Second-half JS Mean +/- Std")
    left_ax.set_xlabel("token index (-1 = last input)")
    left_ax.set_ylabel("JS mean")
    ...
    for category in CATEGORY_ORDER:
        ...
        std_mean_value = summary.get("js_std_mean")
        bar_heights.append(0.0 if mean_value is None else mean_value)
        bar_errors.append(0.0 if std_mean_value is None else std_mean_value)
    ...
    right_ax.set_title("Category JS Mean")
    right_ax.set_ylabel("mean(JS mean)")
    ...
    for category in CATEGORY_ORDER:
        ...
        summary_lines.append(
            ...
            f"mean={_format_metric(summary.get('js_mean_mean'))} "
            f"std={_format_metric(summary.get('js_std_mean'))}"
        )
    right_ax.text(
        1.03,
        0.98,
    ...
    # Plot semantics:
    # left: per-token second-half-to-final JS mean with second-half JS std error bars.
    # right: per-category aggregate second-half-to-final JS mean comparison.
...
def build_correct_tool_focus_summary(sample_records: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    correct_token_records = [
        token_record
        for sample_record in sample_records
        if sample_record["correctness"] == "correct"
        for token_record in sample_record["token_records"]
        if token_record["category"] in {"tool", "eoc", "args"}
    ]
    return {
        "tool_vs_others_js_mean": compare_category_against_others(correct_token_records, "tool", "js_mean"),
        "tool_vs_others_js_std": compare_category_against_others(correct_token_records, "tool", "js_std"),
        "category_counts": {
            category: sum(1 for record in correct_token_records if record["category"] == category)
            for category in ("tool", "eoc", "args")
        },
    }
...
def plot_global_comparison(global_summary: Dict[str, Any], output_path: Path) -> None:
    ...
    for offset, subset_name, color in [(-width / 2, "correct", "#2E8B57"), (width / 2, "incorrect", "#B22222")]:
        ...
        for category in CATEGORY_ORDER:
            ...
            std_mean_value = category_summary.get("js_std_mean")
            means.append(0.0 if mean_value is None else mean_value)
            errors.append(0.0 if std_mean_value is None else std_mean_value)
    ...
    ax.set_ylabel("mean(JS mean)")
    ax.set_title("Second-half JS Mean")
...
def plot_correct_category_comparison(global_summary: Dict[str, Any], output_path: Path) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    errors = []
    for category in CATEGORY_ORDER:
        category_summary = global_summary.get("correct", {}).get(category, {})
        mean_value = category_summary.get("js_mean_mean")
        std_mean_value = category_summary.get("js_std_mean")
        means.append(0.0 if mean_value is None else mean_value)
        errors.append(0.0 if std_mean_value is None else std_mean_value)
    ...
    ax.set_xticklabels(CATEGORY_ORDER, rotation=15)
    ax.set_ylabel("mean(JS mean)")
    ax.set_title("Correct Samples Only")
    ax.grid(alpha=0.2, linestyle="--", axis="y")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Re-evaluate a trained compositional checkpoint with online second-half-to-final JS analysis")
    ...
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory (default: compositional/runs/js_secondhalf2final_explore/<timestamp>)")
...
def main() -> None:
    ...
    write_json(
        ...
        {
            "analysis_type": "secondhalf_to_final_js_mean",
    ...
    correct_tool_focus_summary = build_correct_tool_focus_summary(sample_records)
    plot_global_comparison(global_summary, output_dir / "plots" / "global_correct_vs_incorrect_secondhalf_js_mean.png")
    plot_correct_category_comparison(global_summary, output_dir / "plots" / "correct_category_secondhalf_js_mean.png")
    ...
    write_json(output_dir / "summaries" / "correct_tool_focus_summary.json", correct_tool_focus_summary)
