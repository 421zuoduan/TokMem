...
def train_task_calling_model(model, dataloader, val_dataloader=None, num_epochs=3, lr=0.01, 
    ...
    for epoch in range(num_epochs):
        for batch_idx, batch in enumerate(dataloader):
            ...
            if (batch_idx + 1) % 100 == 0:
                ...
                avg_logit_bias_loss = batch_logit_bias_loss / batches_since_log if batches_since_log > 0 else 0.0
                avg_task_count = batch_task_count / batches_since_log
                
                # Show averages over the logging window
                if batch_task_count > 0:
                    if use_logit_bias:
                        print(f"Epoch {epoch+1}/{num_epochs}, Batch {batch_idx+1}/{len(dataloader)}, Avg Loss: {avg_loss:.4f}, Avg Task Loss: {avg_task_loss:.4f}, Avg Logit Bias Loss: {avg_logit_bias_loss:.4f}, Avg Task Tokens: {avg_task_count:.1f}, LR: {current_lr:.6f}")
                        training_logger.info(f"E{epoch+1}/{num_epochs} B{batch_idx+1}/{len(dataloader)} AvgLoss:{avg_loss:.4f} AvgTaskLoss:{avg_task_loss:.4f} AvgLogitBiasLoss:{avg_logit_bias_loss:.4f} AvgTokens:{avg_task_count:.1f} LR:{current_lr:.6f}")
                    else:
                    ...
                    if use_logit_bias:
                        print(f"Epoch {epoch+1}/{num_epochs}, Batch {batch_idx+1}/{len(dataloader)}, Avg Loss: {avg_loss:.4f}, Avg Task Loss: N/A (no task tokens in window), Avg Logit Bias Loss: {avg_logit_bias_loss:.4f}, LR: {current_lr:.6f}")
                        training_logger.info(f"E{epoch+1}/{num_epochs} B{batch_idx+1}/{len(dataloader)} AvgLoss:{avg_loss:.4f} AvgTaskLoss:N/A AvgLogitBiasLoss:{avg_logit_bias_loss:.4f} LR:{current_lr:.6f}")
                    else:
            ...
            if val_dataloader is not None and validate_every_n_steps is not None and validate_every_n_steps > 0:
                if step % validate_every_n_steps == 0:
                    ...
                    if use_logit_bias:
                        print(f"Step {step} - Validation Loss: {avg_val_loss:.4f}, Avg Logit Bias Loss: {avg_val_logit_bias_loss:.4f}")
                        training_logger.info(f"VALIDATION STEP {step} Loss:{avg_val_loss:.4f} AvgLogitBiasLoss:{avg_val_logit_bias_loss:.4f}")
                    else:
        ...
        if val_dataloader is not None:
            ...
            avg_val_logit_bias_loss = val_metrics['avg_logit_bias_loss']
            if use_logit_bias:
                print(f"Epoch {epoch+1}/{num_epochs} - Validation Loss: {avg_val_loss:.4f}, Avg Logit Bias Loss: {avg_val_logit_bias_loss:.4f}")
                training_logger.info(f"VALIDATION E{epoch+1}/{num_epochs} Loss:{avg_val_loss:.4f} AvgLogitBiasLoss:{avg_val_logit_bias_loss:.4f}")
            else:
    ...
    if task_token_count > 0:
        ...
        if use_logit_bias:
            training_logger.info(f"TRAINING COMPLETE - AvgLoss:{avg_total_loss:.4f} TaskLoss:{avg_task_loss:.4f} AvgLogitBiasLoss:{avg_total_logit_bias_loss:.4f} TaskTokens:{task_token_count} TaskBatches:{task_loss_batches}/{step}")
        else:
        ...
        if use_logit_bias:
            training_logger.info(f"TRAINING COMPLETE - AvgLoss:{avg_total_loss:.4f} AvgLogitBiasLoss:{avg_total_logit_bias_loss:.4f} WARNING:NoTaskTokens")
        else:
